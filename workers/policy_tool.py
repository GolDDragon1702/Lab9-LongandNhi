...
def analyze_policy(task: str, chunks: list) -> dict:
    ...
    if "31/01" in task_lower or "30/01" in task_lower or "trước 01/02" in task_lower:
        policy_version_note = "Đơn hàng đặt ngày trước 01/02/2026 áp dụng chính sách v3 (không có trong tài liệu hiện tại)."
        exceptions_found.append({
...
def run(state: dict) -> dict:
    ...
    try:
        ...
        if needs_tool and any(kw in task.lower() for kw in ["ticket", "p1", "jira", "sự cố"]):
            mcp_result = _call_mcp_tool("get_ticket_info", {"ticket_id": "P1-LATEST"})
            state["mcp_tools_used"].append(mcp_result)
            state["history"].append(f"[{WORKER_NAME}] called MCP get_ticket_info")
            if "exceptions_found" not in policy_result:
                policy_result["exceptions_found"] = []
            policy_result["exceptions_found"].append({
                "type": "ticket_info",
                "rule": f"Ticket Info: {mcp_result.get('output', {})}",
                "source": "mcp_tool:get_ticket_info"
            })

        # Step 4: Access Control tool check
        if needs_tool and any(kw in task.lower() for kw in ["cấp quyền", "access", "level"]):
            level = 1
            if "level 3" in task.lower(): level = 3
            elif "level 2" in task.lower(): level = 2
            
            is_emergency = any(kw in task.lower() for kw in ["emergency", "khẩn cấp", "p1", "sự cố"])
            ...
            if "exceptions_found" not in policy_result:
                policy_result["exceptions_found"] = []
            notes = mcp_result.get("output", {}).get("notes", [])
            for note in notes:
                policy_result["exceptions_found"].append({
                    "type": "access_control_rule",
                    "rule": note,
                    "source": "mcp_tool:check_access_permission"
                })
