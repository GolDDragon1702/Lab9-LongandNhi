...
DOCS_DIR = this_path.parent / "data" / "docs"
...
def preprocess_document(raw_text: str, filepath: str) -> Dict[str, Any]:
    ...
    cleaned_text = "\n".join(content_lines)
    cleaned_text = re.sub(r"\r", "", cleaned_text)              # remove CR
    cleaned_text = re.sub(r"[ \t]+", " ", cleaned_text)         # collapse spaces
    cleaned_text = re.sub(r"\n{3,}", "\n\n", cleaned_text)      # max 2 newlines
    cleaned_text = re.sub(r"[^\S\n]+", " ", cleaned_text)       # trim weird spaces
...
def build_index(collection_name: str = "day09_docs",docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    """
    ...
    # 1. Init components (QUAN TRỌNG: chỉ init 1 lần)
    embedder = LocalEmbedder()
    store = EmbeddingStore(persist_path=str(db_dir))
    ...
    for i, filepath in enumerate(doc_files):
        ...
        for i, chunk in enumerate(chunks):
            ...
            documents.append(
                Document(
                    id=chunk_id,
                    embeddings=embedder.encode(chunk["text"]),
                    content=chunk["text"],
                    metadata={
                        **chunk["metadata"],
        ...
        # 5. Store in ChromaDB
        store.add_documents(collection_name, documents)
